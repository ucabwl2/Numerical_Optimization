function [x, Qx, mse, xiPsi] = fista(b, F, G, lambda, x0, stop, tol, maxit, xt, iPsi)
% FISTA Fast Iterative shrinkage-thresholding algorithm 
%  [x, Qx, mse, xiPsi] = fista(b, F, G, lambda, x0, stop, tol, maxit, xt, iPsi)
%  Solves
%   argmin Q(x) = F(x,b) + lambda*G(x)
%  with Fast Iterative shrinkage-thresholding algorithm (Beck, Teboulle '09).
%
%  INPUTS:
%   b      - data vector
...
% Step size t for gradient step on F (as in ISTA)
t = 1/F.L;
% Start with y = x 
y = x;
% FISTA extrapolation parameter (s-1)/snew
s = 1;

for j = 2:maxit
  % ISTA step at y:
  % Gradient step for F evaluated at y
  xnew = y - t*F.df(y,b);
  % Proximal step wrt G
  xnew = G.prox(xnew,t*lambda);
  
  % New FISTA extrapolation parameter
  snew = 1/2*(1 + sqrt(1 + 4*s^2));
  % New FISTA extrapolated evaluation point 
  y = xnew + (s - 1)/snew * (xnew - x);
