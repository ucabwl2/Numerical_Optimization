...
[xSteep, fSteep, nIterSteep, infoSteep] = descentLineSearch(F, 'newton', lsFun, alpha0, x0, tol, maxIter)
xMin1 = xSteep(1);
xMin2 = xSteep(2);
...
ylabel('y')
zlabel('trajectories')
title('Newton Method trajectories')

error=[];
for j =1:size(infoSteep.xs,2)
    errorTemp = norm(infoSteep.xs(:,j) - xSteep)^2;
...
ylabel('error')
% title('Newton Method using x0= [1.2; 1.2]')
title('Newton Method using x0= [-1.2; 1]')
